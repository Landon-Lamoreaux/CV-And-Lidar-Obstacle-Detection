...
# Define upper and lower HLS Thresholds
lower1 = array([70,150,0])
upper1 = array([130,255,255])
# Define upper and lower HLS Thresholds
lower1 = array([70,150,0])
upper1 = array([130,255,255])

lower2 = array([0,180,0])
upper2 = array([50,210,50])


rclpy.init(args=sys.argv)
node = rclpy.create_node('cameraProcessing')
pub1 = node.create_publisher(PointCloud2, 'camera1Pre', 10)
pub2 = node.create_publisher(PointCloud2, 'camera2Pre', 10)
# r = rclpy.Rate(30)
r = node.create_rate(30)
...
def point_cloud(points, parent_frame):
    ...
    header = std_msgs.Header(frame_id=parent_frame, stamp=node.get_clock().now().to_msg())
...
# Automatically determine the lower and upper hysteresis threasholds for the canny edge detection function.
...
def processCamera(frames, pub, sub_name):
    ...
    # canny = Canny(gray, 50, 150, L2gradient=True)
    canny = auto_canny(gray)
    
    # Dilating the lines to make them thicker.
    canny = dilate(canny, kernel, iterations=1)
    inrange = dilate(inrange, kernel, iterations=2)
    canny = dilate(canny, kernel, iterations=1)
    inrange = dilate(inrange, kernel, iterations=2)
    ...
    masked = dilate(masked, kernel, iterations=1)
...
try:
    while 1:
        ...
        processCamera(frames1, pub1, "camera1")
        processCamera(frames2, pub2, "camera2")
